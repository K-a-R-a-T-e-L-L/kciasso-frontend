...
export default async function Page() {
  ...
  try { admin = await getOptionalAdmin(); } catch (error) {
    if (isAdminApiTransportError(error)) return <AdminBackendUnavailable retryHref="/admin" />;
    throw error;
  }
  if (!admin) redirect("/admin/login");
  if (admin.role === "SUPER_ADMIN" || admin.canManageNews) redirect("/admin/news");
  if (admin.documentsAccessMode === "ALL" || admin.documentGroups.length > 0) redirect("/admin/documents");
  if (admin.canManageSiteSettings) redirect("/admin/settings");
