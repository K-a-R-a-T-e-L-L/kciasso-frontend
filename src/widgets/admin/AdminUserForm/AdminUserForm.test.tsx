...
describe("AdminUserForm permissions", () => {
  ...
  const renderForm = (props: React.ComponentProps<typeof AdminUserForm>) => render(<MantineProvider><AdminUserForm {...props} /></MantineProvider>);

  it("applies full content preset without granting SUPER_ADMIN", async () => {
    renderForm({ includePassword: true, action, submitLabel: "Создать" });
    await userEvent.click(screen.getByRole("button", { name: "Полный доступ к контенту" }));
    expect(screen.getByRole("textbox", { name: "Роль" })).toHaveValue("Admin");
  ...
  it("warns that empty selected document groups grant no access", async () => {
    renderForm({ action, submitLabel: "Сохранить" });
