...
import HomeCarouselEditor from "@/widgets/admin/HomeCarouselEditor/HomeCarouselEditor.client";
import type {
  AdminPageLayout,
  AdminPageSection,
      ...
      <Group justify="space-between" align="flex-start" wrap="wrap">
        <Group gap="sm" align="flex-start">
          ...
          <Stack gap={3}>
            <Group gap="xs">
              <Text fw={700}>
                {index + 1}. {model.friendlyTitle}
              </Text>
              <Badge variant="light" color={model.badgeColor}>
                {model.badgeLabel}
              </Badge>
            </Group>
        ...
        <Group gap="xs">
          ...
          />
          {actions.edit ? (
            <Button variant="subtle" onClick={onEdit} disabled={disabled}>
              Изменить
            </Button>
          ) : null}
          {actions.remove ? (
          ...
          {section.systemRendererKey === "home.carousel" ? (
            <HomeCarouselEditor disabled={disabled} />
          ) : null}
        </Group>
      </Group>
      {actions.preview ? (
        ...
        <Text mt="xs" size="sm" c="dimmed">
          Системный раздел отображается на публичной странице
        </Text>
      )}
    </Card>
  );
...
export default function AdminPageLayoutEditor({
  ...
  const [sections, setSections] = useState(() =>
    [...initialData.sections].sort((a, b) => a.sortOrder - b.sortOrder),
  );
  ...
  const saveOrder = async (
    next: AdminPageSection[],
    previous: AdminPageSection[],
  ) => {
    setSections(next);
    setBusy(true);
    setError("");
    try {
      ...
      if (reason instanceof StaleLayoutError) {
        const message =
          "Страница была изменена в другой вкладке. Загружен актуальный порядок.";
        setError(message);
        ...
        const message =
          reason instanceof Error
            ? reason.message
            : "Не удалось сохранить порядок. Порядок восстановлен.";
        setError(message);
  ...
  const toggle = async (section: AdminPageSection) => {
    ...
    try {
      await mutate(
        endpoint(pageKey, `sections/${section.placementId}/toggle`),
        "POST",
        {
  ...
  const saveCustom = async () => {
    ...
    try {
      ...
      setError(
        reason instanceof Error
          ? reason.message
          : "Не удалось сохранить HTML-секцию",
      );
    } finally {
      setBusy(false);
    }
  ...
  const removeCustom = async (section: AdminPageSection) => {
    ...
    try {
      await mutate(
        endpoint(pageKey, `sections/${section.placementId}`),
        "DELETE",
        { expectedRevision: revision },
      );
    } catch (reason) {
      setError(
        reason instanceof Error ? reason.message : "Не удалось удалить секцию",
      );
    } finally {
      setBusy(false);
    }
  ...
  return (
    <Stack data-testid="page-layout-editor" gap="lg">
      <Group justify="space-between" align="flex-start">
        <Stack gap={3}>
          <Text size="xs" fw={800} tt="uppercase" c="kciassoBlue.6">
            Редактор страниц
          </Text>
          <Title order={2}>Секции страницы: {initialData.title}</Title>
          <Text c="dimmed">
            {initialData.routePattern} · {sections.length} секций
          </Text>
        </Stack>
        {canManageCustomHtml ? (
          <Button onClick={() => beginEdit()} disabled={busy}>
            Добавить HTML-секцию
          </Button>
        ) : null}
      </Group>
      {error ? (
      ...
      {sections.length === 0 ? (
        <Paper p="xl">
          <Text c="dimmed">Для этой страницы пока нет сохранённых секций.</Text>
        </Paper>
      ) : (
        <DndContext
          id="page-layout-editor-dnd"
          sensors={sensors}
          collisionDetection={closestCenter}
          onDragEnd={onDragEnd}
        >
          <SortableContext
            items={sections.map((item) => item.placementId)}
      ...
      >
        <Stack>
          <TextInput
            ...
            value={custom.name}
            onChange={(event) =>
              setCustom({ ...custom, name: event.currentTarget.value })
            }
          />
          <Textarea
            label="HTML"
            ...
            value={custom.html}
            onChange={(event) =>
              setCustom({ ...custom, html: event.currentTarget.value })
            }
          />
          <Textarea
            label="CSS"
            ...
            value={custom.css}
            onChange={(event) =>
              setCustom({ ...custom, css: event.currentTarget.value })
            }
          />
          <Textarea
            label="JavaScript"
            ...
            onChange={(event) =>
              setCustom({ ...custom, javascript: event.currentTarget.value })
            }
          />
          <NumberInput
            label="Высота iframe"
            min={120}
            max={4000}
            value={custom.iframeHeight}
            onChange={(value) =>
              setCustom({ ...custom, iframeHeight: Number(value) || 320 })
            }
          />
          <Button onClick={() => void saveCustom()} loading={busy}>
            {editing ? "Сохранить HTML-секцию" : "Создать HTML-секцию"}
          </Button>
        </Stack>
      </Modal>
    </Stack>
