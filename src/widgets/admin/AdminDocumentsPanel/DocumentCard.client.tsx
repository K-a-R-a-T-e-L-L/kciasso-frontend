...
export type DrawerSection =
  "technical" | "edit" | "replace" | "versions" | "share";
...
export default function DocumentCard(props: Props) {
  ...
  const filteredPlacements = props.document.placements.filter((item) =>
    placementTitle(item.sectionKey)
      ...
      .includes(placementQuery.trim().toLocaleLowerCase("ru-RU")),
  );
  ...
  return (
    <Card
      ...
      withBorder
      shadow="xs"
      data-testid={`document-card-${props.document.id}`}
    >
      <Grid align="center" gutter={{ base: "sm", md: "md" }}>
          ...
          <Group wrap="nowrap">
            ...
            <Stack gap={2} miw={0}>
              ...
              <Text size="sm" c="dimmed">
                {props.document.documentNumber || "Без номера"} ·{" "}
                {formatDate(props.document.documentDate)}
              </Text>
            </Stack>
          </Group>
        </Grid.Col>
              ...
              >
                <Popover.Target>
                  <Button
                    ...
                    size="compact-xs"
                    variant="subtle"
                    onClick={() => props.onToggleExpanded(props.document.id)}
                  >
                    + ещё {extra}
                  </Button>
                </Popover.Target>
                <Popover.Dropdown role="dialog" aria-label="Все размещения">
                  <Stack gap="xs" maw={340}>
                    ...
                    {props.document.placements.length > 12 ? (
                      <TextInput
                        size="xs"
                        value={placementQuery}
                        onChange={(event) =>
                          setPlacementQuery(event.currentTarget.value)
                        }
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        requestAnimationFrame(() =>
                          placementsTriggerRef.current?.focus(),
                        );
                      }}
                    >
                      Закрыть
        ...
        <Grid.Col span={{ base: 12, sm: 6, md: 1 }}>
          <Stack gap={1}>
            ...
            >
              {version?.originalFilename ?? "Файл недоступен"}
            </Text>
        ...
        <Grid.Col span={{ base: 12, md: 4 }}>
          <Stack gap="xs" align="flex-end">
            {props.canReorder && props.onMove ? (
              <Group gap={4}>
                <ActionIcon
                  variant="light"
                  aria-label="Вверх"
                  disabled={props.index === 0}
                  onClick={() => props.onMove?.(props.document.id, -1)}
                >
                  <IconArrowUp size={16} />
                </ActionIcon>
                <ActionIcon
                  variant="light"
                  aria-label="Вниз"
                  disabled={props.index === props.orderedLength - 1}
                  onClick={() => props.onMove?.(props.document.id, 1)}
                >
          ...
          requestAnimationFrame(() =>
            cardRef.current
              ?.querySelector<HTMLButtonElement>(
                'button[aria-label="Действия документа"]',
              )
              ?.focus(),
          );
        }}
      ...
      >
        <Stack ref={drawerFocusRef} tabIndex={-1}>
          {props.drawer === "technical" ? (
            <Stack gap="xs">
              <Text>
                <b>ID документа:</b> {props.document.id}
              </Text>
              <Text>
                <b>Статус:</b>{" "}
              ...
              <Text>
                <b>Количество версий:</b> {props.document.versionsCount}
              </Text>
              <Text>
                <b>Создан:</b> {formatDate(props.document.createdAt)}
              </Text>
              <Text>
                <b>Последнее изменение:</b>{" "}
                {formatDate(props.document.updatedAt)}
              </Text>
              {version ? (
                <>
                  <Text>
                    <b>Текущая версия:</b> {version.versionNumber}
                  </Text>
                  <Text>
                    <b>Имя файла:</b> {version.originalFilename}
                  </Text>
                  <Text>
                    <b>MIME-тип:</b> {version.mimeType}
                  </Text>
                  <Text>
                    <b>Размер:</b> {formatSize(version.sizeBytes)}
                  </Text>
                  <Text style={{ overflowWrap: "anywhere" }}>
                    <b>SHA-256:</b> {version.sha256}
                  </Text>
                </>
              ) : (
                <Text c="dimmed">Файл ещё не загружен.</Text>
              )}
              <Text fw={700} mt="sm">
                Размещения
              </Text>
              {props.document.placements.length ? (
                props.document.placements.map((item) => (
                  <Text size="sm" key={item.sectionKey}>
                    • {placementTitle(item.sectionKey)}
                  </Text>
                ))
              ) : (
                <Text size="sm" c="dimmed">
                  Документ пока не размещён в разделах.
                </Text>
              )}
            </Stack>
          ) : null}
          {props.drawer === "edit" ? (
            <DocumentMetadataForm
              ...
              onFieldChange={props.onEditFieldChange}
              placements={props.editingPlacements}
              onOpenPlacement={props.onOpenPlacement}
              onSubmit={(event) =>
                props.onSaveMetadata(event, props.document.id)
              }
              busy={Boolean(props.busy)}
              onCancel={props.onCancelEdit}
            />
          ) : null}
          {props.drawer === "share" && version ? (
            <DocumentShareLinks version={version} />
          ) : null}
          {props.drawer === "replace" || props.drawer === "versions" ? (
            <DocumentVersionPanel
              ...
              onUpload={(event) =>
                props.onUploadVersion(event, props.document.id)
              }
