...
export default function DocumentCard({ document, index, orderedLength, expanded, editing, versionDocumentId, shareDocumentId, historyDocumentId, versionFile, history, editingForm, editingPlacements, busy, onToggleExpanded, onMove, onOpenFile, onEdit, onToggleVersion, onHistory, onShare, onDelete, onStatusChange, onEditFieldChange, onOpenPlacement, onSaveMetadata, onCancelEdit, onVersionFileChange, onUploadVersion, onCloseHistory, onMakeCurrent }: Props) {
  ...
  return <article className={cls.card} data-testid={`document-card-${document.id}`}>
    <div className={cls.cardHeader}>
      ...
      <select value={document.status === "PUBLISHED" ? "PUBLISHED" : "DRAFT"} disabled={Boolean(busy)} onChange={(event) => onStatusChange(document.id, event.target.value as "DRAFT" | "PUBLISHED")} aria-label={`Статус ${document.title}`}>
    ...
    {shareDocumentId === document.id && version ? <div className={cls.sharePanel}><DocumentShareLinks version={version} /></div> : null}
    <details className={cls.technical}><summary>Техническая информация</summary><p>ID документа: {document.id} · версий: {document.versionsCount}</p><p>{version ? <>Версия {version.versionNumber} · {version.mimeType} · SHA-256: <code>{version.sha256}</code></> : "Текущая версия не определена."}</p></details>
    {editing ? <DocumentMetadataForm mode="edit" form={editingForm} onFieldChange={onEditFieldChange} placements={editingPlacements} onOpenPlacement={onOpenPlacement} onSubmit={(event) => onSaveMetadata(event, document.id)} busy={Boolean(busy)} onCancel={onCancelEdit} /> : null}
    <DocumentVersionPanel documentId={document.id} versionDocumentId={versionDocumentId} versionFile={versionFile} historyDocumentId={historyDocumentId} history={history} busy={Boolean(busy)} onFileChange={onVersionFileChange} onUpload={onUploadVersion} onCloseHistory={onCloseHistory} onMakeCurrent={onMakeCurrent} />
  </article>;
}
