...
export default function useAdminDocumentMutations({ sectionKey, documents, orderedDocuments, form, file, placements, editingPlacements, editingForm, versionFile, history, setDocuments, setForm, setFile, setPlacements, setEditingId, setVersionFile, setVersionDocumentId, setHistory, setBusy, setMessage, setCreateOpen, refresh }: Props) {
  ...
  function makeCurrent(id: number, version: DocumentVersionDto) {
    modals.openConfirmModal({
      title: `Сделать версию ${version.versionNumber} текущей?`,
      children: "Именно эта версия станет доступна для скачивания.",
      labels: { confirm: "Сделать текущей", cancel: "Отмена" },
      onConfirm: () => void makeCurrentConfirmed(id, version),
    });
  }
  async function makeCurrentConfirmed(id: number, version: DocumentVersionDto) { setBusy(`current-${id}`); try { await parseResponse(await fetch(`/api/admin/documents/${id}/versions/${version.id}/current`, { method: "POST" })); setHistory((current) => ({ ...current, [id]: (current[id] ?? []).map((item) => ({ ...item, isCurrent: item.id === version.id })) })); await refresh(); setMessage({ type: "success", text: "Текущая версия изменена." }); } catch (error) { setMessage({ type: "error", text: error instanceof Error ? error.message : "Не удалось выбрать версию." }); } finally { setBusy(null); } }
  function deleteDocument(id: number) {
    modals.openConfirmModal({
      title: "Удалить документ полностью?",
      children: "Будут удалены документ, все его версии и физические файлы. Действие нельзя отменить.",
      labels: { confirm: "Удалить документ", cancel: "Отмена" },
      confirmProps: { color: "red" },
      onConfirm: () => void deleteDocumentConfirmed(id),
    });
  }
  async function deleteDocumentConfirmed(id: number) { setBusy(`delete-${id}`); setMessage(null); try { await parseResponse(await fetch(`/api/admin/documents/${id}`, { method: "DELETE" })); setDocuments((current) => current.filter((item) => item.id !== id)); setMessage({ type: "success", text: "Документ полностью удалён." }); } catch (error) { setMessage({ type: "error", text: error instanceof Error ? error.message : "Не удалось удалить документ." }); } finally { setBusy(null); } }
