...
} from "@/shared/api/adapters/public-documents.adapter";
...
function requisites(number?: string | null, date?: string | null) {
  ...
  if (number && formattedDate)
    return `Документ № ${number} от ${formattedDate}`;
...
export default function PublicDocumentsBlock({
  ...
  const search = (
    <SearchField
      ...
      placeholder="Название, номер, дата или описание"
    />
  );
  ...
  if (result.error) {
    ...
    return (
      <DocumentList
        groups={[]}
        compactEmpty
        emptyTitle="Не удалось загрузить документы. Попробуйте обновить страницу позднее."
        emptyDescription=""
      />
    );
  }
    ...
    return (
      <div className={cls.block} data-document-variant={variant}>
        ...
        <DocumentList
          groups={[]}
          compactEmpty
          emptyTitle="Документы пока не опубликованы."
          emptyDescription=""
        />
      </div>
    );
    ...
    return (
      <div className={cls.block} data-document-variant={variant}>
        ...
        <DocumentList
          groups={[]}
          compactEmpty
          emptyTitle="По вашему запросу документы не найдены."
          emptyDescription=""
        />
      </div>
    );
        ...
        groups={[
          {
            ...
            items: filteredDocuments.map((document) => {
              ...
              return {
                ...
                description: [
                  document.description,
                  requisites(document.documentNumber, document.documentDate),
                ]
                  .filter(Boolean)
                  .join(" · "),
                url: publicDocumentFileUrl(document.id),
                downloadUrl: publicDocumentDownloadUrl(document.id),
                extension,
